...
int CALLBACK TESTEPROC(HWND hDlgHwnd, UINT uMsg, LPARAM lParam){
    HWND hDlg = PropSheet_GetTabControl(hDlgHwnd);
    switch(uMsg){
        case PSCB_PRECREATE:{
            //DLGTEMPLATEEX dlgTemplate = lParam;
            break;
        }
        case PSCB_INITIALIZED:{
        EnumChildWindows(hDlg,mifuck, (LPARAM)NULL);
             //wchar_t **parameters = (wchar_t**)lParam;//"MANAGER" or "CLIENT", profile
             wchar_t *parameters[2]={L"MANAGER", L"Client.ini"};
             iniparser_t iniparser;
             if(iniparser_init(&iniparser)!=0){
                break;
             }
        ...
        case PSCB_BUTTONPRESSED:{
             if(lParam == PSBTN_OK){
                ...
                iniparser_destroy(&iniparser);
                return TRUE;
             }else if (lParam == PSBTN_CANCEL || lParam == PSBTN_FINISH){
                return FALSE;
             }
            break;
        }
...
INT_PTR WINAPI OpenPreferencesDialog(HWND hWndParent, LPARAM dwInitParam){
    ...
    psp[3].pszTitle    = L"3";
    psp[3].lParam      = dwInitParam;
    psp[3].pfnCallback = NULL; //PSP_USECALLBACK
